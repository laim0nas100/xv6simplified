...
kernel: $(OBJS) sys/entry.o initcode sys/kernel.ld
	$(LD) $(LDFLAGS) -T sys/kernel.ld -o kernel sys/entry.o $(OBJS) -b binary initcode
...
kernelmemfs: $(MEMFSOBJS) entry.o initcode fs.img
	$(LD) $(LDFLAGS) -Ttext 0x100000 -e main -o kernelmemfs entry.o  $(MEMFSOBJS) -b binary initcode fs.img
...
ULIB = build/ulib.o build/usys.o build/umalloc.o
...
clean: 
	...
	bootblock initcode initcode.out kernel xv6.img fs.img kernelmemfs mkfs \
	.gdbinit $(UPROGS)
...
QEMUOPTS = -hdb fs.img -drive format=raw,file=xv6.img -smp $(CPUS) -m 512 $(QEMUEXTRA)
